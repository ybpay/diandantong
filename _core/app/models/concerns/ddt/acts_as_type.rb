module Ddt
  module ActsAsType
    ...
    module ClassMethods
      ...
      def acts_as_type_hash(column, hash)
        type_values = []
        type_names = []
        hash.each{ |k,v|
          type_values << k
        ...
        acts_as_type(column, type_values, type_names)
      end

      def acts_as_type(column, type_values=[], type_names=[], options={})
        type_columns << column
        type_values.map!(&:to_sym)
        type_names = type_values if type_names.blank? || type_names.empty?
        raise "acts_as_type values size not equal to names size." if type_values.size != type_names.size
        if options[:default].present?
          define_method column do
            read_attribute(column) or options[:default]
          end
        end
