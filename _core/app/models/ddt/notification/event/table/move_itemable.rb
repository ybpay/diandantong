module Ddt
  class Notification
    module Event
      module Table
        class MoveItemable < Ddt::Notification::Event::Table::Base
          ...
          def action_types_of_target(target_type, target)
            ...
            if target_type == :account
              [:backend, :system_weixin, :app].each do |action|
                account_action_types << action if target.notification_receive_setting.need_notify?(:table_move_itemable, action)
              end
            end
            {
