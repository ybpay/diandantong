module Ddt
  class Notification
    module Event
      module Shipment
        class Started < Ddt::Notification::Event::Shipment::Base
          # 配送开始
          ...
          def action_types_of_target(target_type, target)
            ...
            if target_type == :account
              [:system_weixin, :app].each do |action|
                account_action_types << action if target.notification_receive_setting.need_notify?(:shipment_started, action)
              end
            end
            {
