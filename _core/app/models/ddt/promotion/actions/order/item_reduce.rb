module Ddt
  class Promotion
    module Actions
      module Order
        class ItemReduce < ::Ddt::PromotionAction
          ...
          validates :preferred_amount, numericality: { greater_than_or_equal_to: 0 }
          ...
          concerning :AdjustSource do
            def compute_amount_of_adjustment(order)
              ...
              amount = items.map(&:active_quantity).sum * preferred_amount * -1
              amount.round(2)
            end
            ...
            def get_item_adjustments(order)
              ...
              items.map do |item|
                item_reduce_amount = -(item.active_quantity * preferred_amount).round(2)
                item.get_item_adjustment(item_reduce_amount)
              end
            end
          end
        end
