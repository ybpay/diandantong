module Ddt
  module Bill
    module Branch
      class AntiSettlementList < Ddt::Bill::Branch::Base
        ...
        def items
          @items ||= OrderService::Api::Statistic.order_change_list(query: {
              type_eq: "Ddt::OrderChangeLog::AntiSettlement",
              branch_id_eq: @branch.id,
              created_at_gteq: start_time,
              created_at_lteq: end_time
            }).map{|item| AntiSettlementList::Item.new(item)}
        end

        class Item
          attr_accessor :type, :created_at, :description, :operator_name, :order_number, :order_type, :order_total, :order_table_name, :order_table_zone_name, :order_id
