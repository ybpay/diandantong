...
module Ddt
  module Preferences
    ...
    class StoreInstance
      ...
      def exist?(key)
        @cache.exist?(key) ||
        should_persist? && Ddt::Preference.where(:key => key).exists?
      end
      ...
      def get(key)
        ...
        if should_persist?
          ...
          if preference = Ddt::Preference.find_by_key(key)
            # it does exist
      ...
      def persist(cache_key, value)
        ...
        preference = Ddt::Preference.where(:key => cache_key).first_or_initialize
      ...
      def destroy(cache_key)
        ...
        preference = Ddt::Preference.find_by_key(cache_key)
        preference.destroy if preference
      end

      def should_persist?
        @persistence and Ddt::Preference.table_exists?
      end
