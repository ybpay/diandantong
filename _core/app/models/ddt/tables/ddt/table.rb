module Ddt
  class Table < Ddt::Base
    ...
    state_machine :workflow_state, initial: :idle do
      event :open do
        transition [:idle, :opened] => :opened
      end
      event :order do
        transition [:idle, :opened] => :ordered
      end
      event :clear do
        transition [:opened, :paid] => :idle
      end
      event :pay do
        transition [:ordered, :check_outing] => :paid
      end
      event :cancel do
        transition :ordered => :idle
      end
      event :check_out do
        transition :ordered => :check_outing
      end
      event :cancel_check_out do
        transition :check_outing => :ordered
      end
      event :anti_settlement do
        transition [:idle, :opened, :paid] => :ordered
      end
      event :force_clear do
        transition [:opened, :ordered, :check_outing, :paid] => :idle
      end

      state :idle, :opened do
        def active?
          false
        end
      ...
      state :ordered, :paid, :check_outing do
        def active?
          true
        end
