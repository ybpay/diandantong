module Ddt
  class CheckBranchAdjustmentAmountWorker
    include Sidekiq::Worker
    sidekiq_options :retry => 3, :queue => :default

    def perform(branch_id, start_time, end_time)
      interval = Time.parse(start_time)..Time.parse(end_time)
      errors = []
      Ddt::Order.where(
        placed_at: interval,
        pay_item_state: :paid,
      ...
      ).pluck(:id).each_slice(50).to_a.each do |ids|
      ...
      if errors.present? && Rails.env.production?
        BaseMailer.notify(
          ...
          "[faigo 数据检查] 门店#{Ddt::Branch.find(branch_id).try(:name)}",
          errors.join("\n")
        )
      end
    end
