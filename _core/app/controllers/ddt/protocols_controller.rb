...
module Ddt
  class ProtocolsController < ActionController::Base
    ...
    def index
      @agent = Ddt::Agent.with_deleted.find_by(agent_no: params[:agent_no]) if params[:agent_no].present?
      @agent = Ddt::Agent.with_deleted.find_by(domain: request.host) unless [Ddt::Host::DEPLOY, Ddt::Host::LOCAL].include? request.host
