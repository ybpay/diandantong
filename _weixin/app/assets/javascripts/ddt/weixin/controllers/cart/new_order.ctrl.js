Ddt.controller('newDeliveryOrderController', [
    ...
    function ($rootScope, $scope, $location, $timeout, BaseNewOrderController, AddressService, DeliveryCartService, DeliveryOrderService, UserService, $routeParams) {
      ...
      BaseNewOrderController.action($scope, function() {
        ...
        var distance_of_ab = function(a, b){
        ...
        $scope.is_distance_of_branch_to_address_valid = function(address){
          var errors = []
          // 国内用户可能需要拿user的地理位置
          if(!$scope.enable_foreign && !(address.latitude && address.longitude)){
            address.latitude = $scope.user.last_latitude;
            address.longitude= $scope.user.last_longitude;
          }
          var distance = distance_of_ab($scope.branch, address);
          if($scope.enable_foreign && distance == undefined && !$scope.branch.support_order_if_not_in_delivery_radius){
            errors.push("请编辑您的收货地址，并为其标注位置");
          }
          if(distance && distance > ($scope.branch.delivery_radius * 1000) && !$scope.branch.support_order_if_not_in_delivery_radius ){
            errors.push('您的收货地址已经超过我们的配送范围: '+$scope.branch.delivery_radius+' 公里');
          }
          return errors;
        }
        ...
        $scope.submit = function () {
          ...
          if ($scope.can_submit()) {
            ...
            DeliveryOrderService.create($scope.branch_id, {
              ...
              $location.path('/branches/' + $scope.branch_id + '/order_success/'+ resp.order.id + '/delivery').replace();
  ...
  function ($rootScope, $scope, $timeout, BaseNewOrderController, newOrderInHallController, FastfoodOrderService){
    ...
    function ($rootScope, $scope, $location, $timeout, BaseNewOrderController, ReservationOrderService, AddressService, UserService) {
      ...
      BaseNewOrderController.action($scope, function () {
        ...
        $scope.initialized = true;
        console.log($scope.input_pay_method)
      })

      $scope.note = null

      $scope.prepay_total = function () {
        if ($scope.cart && $scope.prepayment_type == 'prepay_for_table') {
          return $scope.cart.table_zone.reservation_price
        } else if ($scope.cart && $scope.prepayment_type == 'prepay_for_order') {
          return $scope.cart.total
        }else{
          console.info("$scope.cart is not ready")
        }
      }
      ...
      $scope.submit = function () {
        ...
        if ($scope.can_submit()) {
          ...
          ReservationOrderService.create($scope.branch_id, {
            ...
            $rootScope.is_submiting = false
            $location.path('/branches/' + $scope.branch_id + '/order_success/'+ resp.order.id + '/reservation').replace();
          })
        }
      }
    ...
    '$rootScope', '$scope', '$timeout', '$location', 'BaseNewOrderController', 'GrouponOrderService',
    function ($rootScope, $scope, $timeout, $location, BaseNewOrderController, GrouponOrderService) {
      $scope.self = $scope;
      ...
      $scope.submit = function () {
        ...
        if ($scope.can_submit()) {
          ...
          GrouponOrderService.create($scope.branch_id, {
            ...
            $rootScope.is_submiting = false
            $location.path('/branches/' + $scope.branch_id + '/order_success/'+ resp.order.id + '/groupon').replace();
          })
        }
      }
      ...
      $scope.submit = function () {
        ...
        if ($scope.can_submit()) {
          ...
          RechargeOrderService.create($scope.branch_id, {
            ...
            $rootScope.is_submiting = false
            $rootScope.go_page('order', '/branches/' + $scope.branch_id + '/orders/' + $scope.cart_type + '/' + resp.order.id +'/pay_online')
          })
        }
      }
    }])
