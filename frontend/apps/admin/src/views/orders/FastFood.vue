  ...
  <div class="p-6 space-y-4">
    ...
    <el-card shadow="never">
      <el-form :inline="true" :model="searchForm">
        ...
        <el-form-item label="取餐码">
          <el-input v-model="searchForm.take_no" placeholder="取餐码" clearable style="width: 120px" />
        </el-form-item>
    ...
    <el-card shadow="never">
      <el-table :data="orders" stripe v-loading="loading" style="width: 100%">
        ...
        <el-table-column prop="take_no" label="取餐码" width="100">
          <template #default="{ row }">
            <span class="font-bold text-orange-500">{{ row.take_no }}</span>
          </template>
        </el-table-column>
...
const searchForm = reactive({ order_no: '', take_no: '', status: '' })
...
async function fetchOrders() {
  loading.value = true
  try {
    const q: Record<string, unknown> = { order_type_eq: 'fast_food' }
    if (searchForm.status) q.status_eq = searchForm.status
    if (searchForm.order_no) q.order_no_cont = searchForm.order_no
    if (searchForm.take_no) q.take_no_cont = searchForm.take_no
