  ...
  <div class="p-6 space-y-4">
    ...
    <el-card shadow="never">
      <el-form :inline="true" :model="searchForm">
        ...
        <el-form-item label="桌号">
          <el-input v-model="searchForm.table_name" placeholder="桌号" clearable style="width: 120px" />
        </el-form-item>
    ...
    <el-card shadow="never">
      <el-table :data="orders" stripe v-loading="loading" style="width: 100%">
        ...
        <el-table-column prop="table_name" label="桌号" width="80">
          <template #default="{ row }">
            <el-tag type="warning" size="small">{{ row.table_name }}号桌</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="guest_num" label="人数" width="70" />
        <el-table-column label="菜品明细" min-width="250">
          <template #default="{ row }">
            <span>{{ formatLineItems(row.line_items) }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="placed_at" label="下单时间" width="160">
...
async function fetchOrders() {
  loading.value = true
  try {
    const q: Record<string, unknown> = { order_type_eq: 'eat_in_hall' }
    if (searchForm.status) q.status_eq = searchForm.status
    if (searchForm.order_no) q.order_no_cont = searchForm.order_no
    if (searchForm.table_name) q.table_name_cont = searchForm.table_name
