class FixLineItemNotActualAmountData < ActiveRecord::Migration
  def change
    start_time = Time.parse('2016-01-01')
    end_time = Time.parse('2016-10-01')
    dates = (start_time.to_i...end_time.to_i).step(1.day).map{|i| Time.at(i).strftime('%Y-%m-%d')}
    dates.each do |date|
      next_date = date.next
      r = execute <<-SQL
        select line_item.id from
          (select o.number, o.id, sum(l.not_actual_amount) as not_actual_amount from ddt_orders as o
            ...
            where o.pay_item_state = 'paid'
              ...
              group by l.order_id) as line_item
        inner join
          (select o.number, p.pay_method_name, o.id, sum(not_actual_amount) as not_actual_amount from ddt_orders as o
            ...
            where o.pay_item_state = 'paid'
                ...
                and p.state = 'paid'
              group by p.order_id
              ) as pay_item on line_item.id = pay_item.id
        where line_item.not_actual_amount != pay_item.not_actual_amount;
      SQL
      ...
      if order_ids.present?
        ...
        order_ids.each_slice(200).each do |oids|
          ...
          Ddt::OrderService::Order::Base.includes(:line_items, :pay_items).each do |order|
