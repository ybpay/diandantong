module Ddt
  module Api
    module V1
      module Backend
        class TableZonesController < Ddt::Api::V1::BaseController
          ...
          def show
            render_resource(@table_zone, serializer: ->(tz) {
              tz.as_json(include: [:tables])
            })
          end
          ...
          def set_table_zone
            @table_zone = @branch.table_zones.find(params[:id])
          end
