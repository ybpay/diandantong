module Ddt
  class Backend::Wallet::UserCardWalletsController < Backend::BaseController
    check_permission :shop, :user, {
      [:get_recharge, :recharge] => :recharge_card_wallet,
      [:get_exchange, :exchange] => :exchange_card_wallet,
      wallet_logs: :show
    }

    include Backend::BaseWalletsController
    before_action :set_wallet, only: [:wallet_logs, :get_recharge, :recharge, :get_exchange, :exchange]
    def get_recharge
