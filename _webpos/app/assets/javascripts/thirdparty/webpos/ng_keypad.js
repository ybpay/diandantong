...
(function () {
    ...
    window.Keypad = function ($scope, $element, $attrs, $rootScope) {
        ...
        function init() {
            ...
            $scope.$on(Keypad.TOGGLE_LOCKING, handleLockingToggle);
            $scope.$on(Keypad.TOGGLE_OPENING, handleOpeningToggle);
            $scope.$on(Keypad.OPEN, handleOpeningToggle);
            $scope.$on(Keypad.CLOSE, handleOpeningToggle);
        ...
        function handleKeyPressed(event, key) {
            if (!locked) {
                if (key.indexOf('[') === -1 && key.indexOf(']') === -1) {
                    $rootScope.$broadcast(Keypad.KEY_PRESSED, key, padId);
                } else {
                    $rootScope.$broadcast(Keypad.MODIFIER_KEY_PRESSED, key.substring(1, key.length - 1), padId);
                }
            }
        }
        ...
        function open() {
            ...
            $rootScope.$broadcast(Keypad.OPENED, padId);
            autoClose();
            applyOptions();
        }
        ...
        function close() {
            opened = false;
            $rootScope.$broadcast(Keypad.CLOSED, padId);
