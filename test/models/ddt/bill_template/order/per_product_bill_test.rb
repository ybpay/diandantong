...
module Ddt
  module BillTemplate
    module Order
      class PerProductBillTest < TestCase::Base
        let(:order){ example_eat_in_hall_order }
        let(:printer){ create(:normal_printer, branch: branch) }
        def test_render
          bill = BillTemplate::Order::PerProductBill.new(order: order, printer: printer).render.first
          assert bill.include?(order.number)
          assert bill.include?(order.line_items.first.name)
        end

        def test_preview
          bill = BillTemplate::Order::PerProductBill.preview(branch)
