...
module Ddt
  module BillTemplate
    module Order
      class AppendPerProductBillTest < TestCase::Base
        let(:order){
        ...
        def test_render
          bill = BillTemplate::Order::AppendPerProductBill.new(order: order, printer: printer, order_change_log: log).render.first
          assert bill.include?(order.number)
          assert bill.include?(variant.name)
        end

        def test_preview
          bill = BillTemplate::Order::AppendPerProductBill.preview(branch)
