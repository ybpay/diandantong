...
module Ddt
  module BillTemplate
    module Shifts
      class ByShiftTest < TestCase::Base
        let(:shift) {
        ...
        let(:shift_list){
          shift
          Bill::Branch::ShiftList.new(branch, operator: worker, search_by: :by_shift, start_time: 2.hours.ago, end_time: 2.hours.since)
        }
        def test_render
          bill = BillTemplate::Shifts::ByShift.new(shift_list, printer).render
          assert bill.include?("交班汇总")
          assert bill.include?("消费人数:1")
          assert bill.include?("现金             10.0")
        end

        def test_preview
          bill = BillTemplate::Shifts::ByShift.preview(branch)
          assert bill.include?("交班汇总")
          assert bill.include?("消费人数:1")
